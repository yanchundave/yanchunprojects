forecast_datasets = ['2021-04-01', '2021-07-01', '2021-10-01', '2022-01-01']
with open("yellow_feature_sql.txt", 'a') as f:
    for item in forecast_datasets:
        ...
        DATEADD('month', 6, DATE('{item}')) AS ENDDATE
        ...
        ON USERS.USER_ID = TRANS.USER_ID
        WHERE DATE(TRANS.TRANS_TIME) >= USERS.FORECASTDATE AND DATE(TRANS.TRANS_TIME) < DATE(USERS.ENDDATE)
        GROUP BY USERS.USER_ID
        )
        SELECT
        USERS.USER_ID,
