...
AND to_date(PV_TS) < DATE('2022-02-01');
...
with USER_TRANS AS
    (
        ...
        WHERE TRANS_TIME IS NOT NULL AND to_date(TRANS_TIME) <= '2022-01-31'
    )
    SELECT
    USER_ID,
    DATE(MIN(TRANS_TIME)) AS first_trans,
    COUNT(TRANS_ID) - 1 AS frequency,
    DATEDIFF('day', DATE(MIN(TRANS_TIME)), DATE('2022-02-01')) AS T,
...
FROM  ACCOUNTING.DBT_LOCAL.USER_TRANSACTION_2022
WHERE TRANS_TIME IS NOT NULL AND to_date(TRANS_TIME) >= date('2022-02-01')
AND to_date(TRANS_TIME) < date('2022-08-01')
GROUP BY USER_ID
