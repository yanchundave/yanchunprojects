...
def main(session: snowpark.Session):
    ...
    for datepar in dateset:
        ...
        df_1 = df.to_pandas()
        df_1[['trending','intercept']] = df_1.apply(lambda row:get_trending(row), axis=1).apply(pd.Series)
        dflist.append(df_1)

    dfcombine = pd.concat(dflist, ignore_index=True)
    ...
    df_output = session.create_dataframe(df_data)
    df_output.write.mode("overwrite").save_as_table("sandbox.dev_yyang.ltv_test", table_type="")
