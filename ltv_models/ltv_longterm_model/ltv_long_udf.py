...
from ltv_long_global import numeric_features, category_features, channels
...
def transform_x(df):
    ...
    x_combine = np.concatenate(
        [
            array_category,
            x,
            np.array(df["REALREVENUE"]).reshape([-1, 1]),
    ...
    dfupdate = pd.DataFrame(
        x_combine,
        index=df["USER_ID"],
        columns=columns_name + ["REALREVENUE", "LG_REVENUE"],
    ).reset_index()
