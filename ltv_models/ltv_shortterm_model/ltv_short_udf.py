...
from ltv_short_global import numeric_features, category_features, channels
...
def transform_x(df):
    ...
    x_combine = np.concatenate(
        [
            array_category,
            x,
            df.loc[:, ["REVENUE", "LG_REVENUE", "FORECASTDATE"]].values,
        ],
        axis=1,
    )
    dfupdate = pd.DataFrame(
        x_combine,
        index=df["USER_ID"],
        columns=columns_name + ["REVENUE", "LG_REVENUE", "FORECASTDATE"],
    ).reset_index()
...
def transform_x_forecast(df, scaler):
    ...
    x_combine = np.concatenate(
        [
            array_category,
            x,
            np.array(df["REVENUE"]).reshape([-1, 1]),
            np.array(df["LG_REVENUE"]).reshape([-1, 1]),
        ],
        axis=1,
    )
    dfupdate = pd.DataFrame(
        x_combine, index=df["USER_ID"], columns=columns_name + ["REVENUE", "LG_REVENUE"]
    ).reset_index()

    x_combine_lr = np.concatenate(
        [
            array_category,
            x_lr,
            np.array(df["REVENUE"]).reshape([-1, 1]),
    ...
    dfupdate_lr = pd.DataFrame(
        x_combine_lr,
        index=df["USER_ID"],
        columns=columns_name + ["REVENUE", "LG_REVENUE"],
    ).reset_index()
