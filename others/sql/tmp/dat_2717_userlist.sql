...
paused_user as (
    ...
    WHERE PAUSED_AT <= DATE('2024-01-01')
...
SELECT
    ...
    case
    when f.user_id is null then 'not_migrated'
    else 'migrated'
    end as if_migrated
...
------Final user list
select user_id
from sandbox.dev_yyang.paused_user
where if_paused_user = 'not_paused'
and if_deleted_or_sub_cancel = 'not_deleted'
and if_billing_cycle_oct = 'no_billing_cycle_oct'
and if_migrated = 'not_migrated'
and healthy_or_unhealthy in ('healthy', 'unhealthy')