...
BANK_CONNECTION_CURRENTUSER AS
(
    SELECT
        A.USER_ID,
    ...
    WHERE A.INITIAL_PULL < DATEADD('DAY', -31, CURRENT_DATE())
        AND B.FIRST_ADVANCE_TIMESTAMP IS NOT NULL
    GROUP BY A.USER_ID
),
CURRENT_MEMBER AS
(
    SELECT
        ...
        B.IS_APPROVED,
        B.IS_DISBURSED
    FROM BANK_CONNECTION_CURRENTUSER A
    LEFT JOIN ANALYTIC_DB.DBT_MARTS.REQUESTS B
    ON A.USER_ID = B.USER_ID
),
CURRENT_MEMBER_GROUP AS
(
    SELECT
        USER_ID,
