CREATE OR REPLACE TABLE DBT.DEV_YANCHUN_PUBLIC.competitors_lightdormant AS
...
(
    SELECT
        ...
        COUNT(b.taken_amount) AS count_value
    FROM userupdate a
    JOIN disbursement b
    ON a.user_id = b.user_id
...
select
  user_id,
  date_of_ref,
  COALESCE(user_label, 'undecided') as user_label,
  advance_source,
  TO_NUMERIC(COALESCE(last_amount, 0)) AS last_amount,
  TO_NUMERIC(COALESCE(max_value, 0)) as max_value,
  TO_NUMERIC(COALESCE(SUM_VALUE, 0)) AS sum_value,
  TO_NUMERIC(COALESCE(count_value, 0)) as count_value
from final