with subscription_payment AS (
	...
	SELECT * FROM {{ source('transactions', 'SUBSCRIPTION_PAYMENT') }}
...
subsc1_refund_details AS (
    SELECT r.created AS refund_created,
           ...
           to_varchar(p.id) as subscription_payment_id,
           p.created as payment_created,
           p.amount as subsc_fee_collected,
           'subs1' as product
      FROM subscription_payment p
           LEFT JOIN reimbursement r
           ON p.id = r.subscription_payment_id
     ...
     -- we've been including both 'PENDING' AND 'COMPLETED' status. But there are also other
     -- scenarios that caused the refunds to stuck at 'PENDING' status, which needs further
     -- reconciliation by eng. But the 'PENDING' refund amount is usually < $10 each month and the
     -- total 'PENDING' amount is only $1.2k as of June 2023, which is not material.
