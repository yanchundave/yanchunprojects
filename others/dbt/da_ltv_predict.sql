...
users_candidate as (
    ...
    (
        select
            distinct user_id
        from disbursements
        where disbursement_ds_pst >= dateadd('month', -6,  date_trunc('MONTH', current_date()))
...
uc_trans as (
    select
        ...
        utf.principal,
        case
        when utf.first_payment_date < date_trunc('MONTH', current_date()) then utf.first_payment_date
        else null
