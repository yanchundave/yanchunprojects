...
users_candidate as (
    ...
    (
        select
            distinct user_id
        from disbursements
        where disbursement_ds_pst >= dateadd('month', -6,  date(?))
            and disbursement_ds_pst < date_trunc('MONTH', date(?))
        union
        select
            distinct user_id
        from settlements
        where last_payment_date is not null and date(last_payment_date) >= dateadd('month', -6,  date_trunc('MONTH', date(?)))
            and date(last_payment_date) < date_trunc('MONTH', date(?))
    )sample (200000 rows)
) ,
...
approved_bank as (
    ...
    where advance_requests.requested_ds <= date_trunc('MONTH', date(?))
    group by 1
),
...
uc_trans_first as (
    select
        ...
        case
        when ds.original_payback_date > date_trunc('MONTH', date(?)) then 1
        else 0
    ...
    where ds.disbursement_ds_pst <= date_trunc('MONTH', date(?))
...
uc_settle as (
    ...
    where us.payment_ds_pst < date_trunc('MONTH', date(?))
    group by 1
),
...
uc_trans as (
    select
        ...
        utf.principal,
        case
        when utf.first_payment_date < date_trunc('MONTH', dateadd('month', -1, date(?))) then utf.first_payment_date
        else null
...
rmf as (
    select
        ...
        datediff('day', date(min(disbursement_ds_pst)), date_trunc('MONTH',date(?))) as T,
        avg(
            case
...
prior_revenue as (
    ...
    where ss.settlement_due_ds_pst >= dateadd('month', -6, date_trunc('MONTH', date(?)))
        and ss.settlement_due_ds_pst < date_trunc('MONTH', date(?))
    group by 1
),
...
revenue as (
    ...
    join settlements ss
    on uc.user_id = ss.user_id
    where ss.settlement_due_ds_pst >= date_trunc('MONTH', date(?))
        and ss.settlement_due_ds_pst < dateadd('month', 6, date_trunc('MONTH', date(?)))
    group by 1
),
...
from final
    ...
    for datepar in dateset:
        df = session.sql(query_str,
                         params=[datepar, datepar,datepar,datepar,datepar,
                                 datepar,datepar,datepar,datepar,datepar,
                                 datepar, datepar, datepar, datepar, datepar
                                 ])
    ...
    for item in dflist[1:]:
        dfcombine = dfcombine.union(item)

    dfcombine.write.mode("overwrite").save_as_table("sandbox.dev_yyang.da_ltv_dataset", table_type="")
    return type(dfcombine)
