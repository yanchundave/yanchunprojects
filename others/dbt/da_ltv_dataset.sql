...
new_user_attribution as (
    ...
    select * , rank() over (partition by user_id order by network) as rnk from {{ ref('new_user_attribution') }}
    qualify rnk = 1
),
...
cte_{{ loop.index }} as (
    ...
    uc_trans_first as (
        select
            ...
            case
            when ds.original_payback_date > date('{{ parameter}}') then 1
            else 0
    ...
    uc_settle as (
        ...
        where us.payment_ds_pst < date('{{ parameter}}')
        group by 1
    ),
