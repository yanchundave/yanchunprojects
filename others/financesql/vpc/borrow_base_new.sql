--Remove pa with the new requirements of DAT-3496
with
...
SELECT
  ...
  -- branch 1
  SUM(IFF(DPD <= -15, remaining_balance - tip_receivables, 0)) AS "Gross Advances - 15 Days or More Delinquent-Past-Due-Date",
  SUM(IFF(DPD > -15, remaining_balance - tip_receivables, 0)) AS "Gross Advances - 14 Days or Less Delinquent-Past-Due-Date",
  --branch 2
  SUM(IFF(advance_principal > 600 AND DPD > -15, remaining_balance - tip_receivables, 0)) AS "Advances in Excess of Size 600",
  SUM(IFF(diff_advance_payback_date < -30 AND DPD > -15 AND advance_principal <= 600, remaining_balance - tip_receivables, 0)) AS "Advances in Excess of Term Criteria",
  ...
  SUM(IFF(multi_advance_outstanding != 1 AND payback_date_modified != 1
      AND advance_principal > 100
      AND diff_advance_payback_date >= -30
      AND advance_principal <= 600
      AND DPD > -15, remaining_balance - tip_receivables, 0)) AS "Eligible Receivables with Advances > 100",

  SUM(IFF(multi_advance_outstanding != 1 AND payback_date_modified != 1
      AND DPD > -15 AND DPD < 0
      AND advance_principal <= 600
      AND diff_advance_payback_date >= -30,
      remaining_balance - tip_receivables, 0)) AS "1-14 DPD - Excess Concentration Limits",
  ...
  SUM(IFF(multi_advance_outstanding != 1 AND payback_date_modified != 1
      ...
      remaining_balance - tip_receivables, 0)) AS "1-14 Payback - Excess Concentration Limits",
  ...
  SUM(IFF(multi_advance_outstanding != 1 AND payback_date_modified != 1
      ...
      AND diff_advance_payback_date <= -15
      AND diff_advance_payback_date >= -30,
      remaining_balance - tip_receivables, 0)) AS "15-30 Payback - Excess Concentration Limits"
FROM final
