...
settlements as (
    ...
    select * from {{ ref('settlements') }}
),

settlement_details as (
    select * from {{ source('overdraft_overdraft', 'SETTLEMENT') }}
),

new_user_attribution as (

    select * from {{ ref('new_user_attribution') }}
),

ltv_bankconnection as (

    select *,
        rank() over (partition by user_id order by has_valid_credentials desc) as ranknum
    from {{ ref('ltv_bankconnection') }}
    qualify ranknum = 1
),
...
user_income as (
    ...
    from
        (
            select *
            from {{ source('income_detection', 'PAYROLL_ASSESSMENT') }}
...
user_settlement as (
    ...
    select
        overdraft_id,
        settlement_ds as payback_date,
        settlement_amount as payment_amount
    from {{ ref('fct_overdraft_settlement') }}

),

{% for parameter in training_dates %}
...
cte_{{ loop.index }} as (
    ...
    users_candidate as (
        ...
        (
            select
                distinct user_id
            from disbursements
            where disbursement_ds_pst >= dateadd('month', -6,  date('{{ parameter}}'))
                and disbursement_ds_pst < date('{{ parameter}}')
            union
            select
                distinct user_id
            from settlements
            where last_payment_date is not null and date(last_payment_date) >= dateadd('month', -6,  date('{{ parameter}}'))
                and date(last_payment_date) < date('{{ parameter}}')
        ) sample (200000 rows)
    ...
    approved_bank as (
        ...
        where advance_requests.requested_ds <= date('{{ parameter}}')
        group by 1
    ),
    ...
    uc_trans_first as (
        select
            ...
            case
            when ss.settlement_due_ds_pst > date('{{ parameter}}') then 1
            else 0
        ...
        where ds.disbursement_ds_pst <= date('{{ parameter}}')
    ...
    uc_settle as (
        select
            ut.advance_id,
            sum(us.payment_amount) as total_settlement_amount
        from uc_trans ut
        left join user_settlement us
        on ut.advance_id = us.overdraft_id
        where us.payback_date < date('{{ parameter}}')
        group by 1
    ),
    ...
    uc_trans as (
        select
            ...
            us.total_settled_amount - ss.principal as net_rev,
            utf.datediff,
            utf.not_fully_baked,
            ...
            utf.principal,
            case
            when utf.first_payment_date < date('{{ parameter}}') then utf.first_payment_date
            else null
    ...
    rmf as (
        select
            ...
            datediff('day', date(min(disbursement_ds_pst)), date('{{ parameter}}')) as T,
            avg(
                case
    ...
    prior_revenue as (
        ...
        where ss.settlement_due_ds_pst >= dateadd('month', -6, date('{{ parameter}}'))
            and ss.settlement_due_ds_pst < date('{{ parameter}}')
        group by 1
    ),
    ...
    revenue as (
        ...
        join settlements ss
        on uc.user_id = ss.user_id
        where ss.settlement_due_ds_pst >= date('{{ parameter}}')
            and ss.settlement_due_ds_pst < dateadd('month', 6, date('{{ parameter}}'))
        group by 1
    ),
    ...
    final as (
        select
            ...
            date('{{ parameter}}') as forecast_date
