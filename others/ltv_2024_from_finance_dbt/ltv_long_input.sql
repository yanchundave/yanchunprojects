...
(
    ...
    FROM {{ source('dbt_metrics', 'LTV_USER') }} USER
...
(
    ...
    FROM {{ source('dbt_marts', 'DIM_ADVANCE_USERS') }} as dau
...
APPROVED_BANK AS (
    ...
    FROM {{ source('dbt_marts', 'FCT_ADVANCE_APPROVALS') }}  AS advance_requests -- included EC
    LEFT JOIN {{ source('dbt_marts', 'FCT_ADVANCES') }}  AS advance_takeout -- legacy advance only
    ON advance_requests.advance_approval_id = advance_takeout.chosen_advance_approval_id
    LEFT JOIN {{ source('dbt_marts', 'FCT_OVERDRAFT_DISBURSEMENT') }} AS o2_takeout -- EC overdraft only
...
(
    ...
    JOIN {{ ref('ltv_transaction') }} TRANS
...
(
    ...
    FROM USER_SUBSET USERS
    JOIN {{ source('dbt_metrics', 'ONE_DAVE_ACTIVE_USERS') }} ACTIVE
...
(
    SELECT
        ...
        UR.REALREVENUE
    FROM USER_SUBSET USERS
    LEFT JOIN {{ source('dbt_metrics', 'LTV_BANKCONNECTION') }} BANKING
