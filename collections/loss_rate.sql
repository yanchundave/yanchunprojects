...
DROP TABLE IF EXISTS DBT.DEV_YANCHUN_PUBLIC.LOSS_RATE_WEEKLY;
CREATE table DBT.DEV_YANCHUN_PUBLIC.LOSS_RATE_WEEKLY AS
...
WITH advance_payments AS (
  ...
  FROM
    ...
    INNER JOIN analytic_db.dbt_marts.dim_dates d ON a.payback_ds = d.event_ds
    CROSS JOIN (SELECT event_ds FROM analytic_db.dbt_marts.dim_dates) f
    LEFT JOIN analytic_db.dbt_marts.fct_payments p ON a.advance_id = p.advance_id
    AND DATEDIFF(DAY, a.payback_ds, f.event_ds) = p.days_since_payback_date
    LEFT JOIN ANALYTIC_DB.DBT_MARTS.FCT_ADVANCE_PAYBACK q ON a.advance_id = q.advance_id
  WHERE
    a.PAYBACK_DS >= DATEADD(WEEK, '-30', '2022-10-24')
...
extra_cash_payments AS (
    ...
    FROM
      ...
      INNER JOIN analytic_db.dbt_marts.dim_dates d ON settlement_due_date = d.event_ds
      CROSS JOIN (SELECT event_ds FROM analytic_db.dbt_marts.dim_dates) f
      LEFT JOIN analytic_db.dbt_marts.fct_overdraft_settlement p ON o.overdraft_id = p.overdraft_id
      AND p.settlement_ds = f.event_ds
      LEFT JOIN (SELECT OVERDRAFT_ID, TO_DATE(SETTLEMENT_DUE_DS) AS payback_date,
    ...
    WHERE
      settlement_due_date >= DATEADD(WEEK, '-30', '2022-10-24')
...
all_payments AS (
SELECT a.*, 'Legacy Advance' AS disbursement_product_type FROM advance_payments a
UNION
SELECT e.*, 'Extra Cash' AS disbursement_product_type FROM extra_cash_payments e
),
...
SELECT * FROM all_cumulative_payments
) AS loss_rate_weekly
  WHERE settlement_due_ds < DATEADD(WEEK, 0, '2022-10-24')
),
