...
"""
Get column name property: ios, android or other
"""
def get_column_property(column_name):
        if 'iOS' in column_name:
                return 'iOS'
        elif 'Android' in column_name:
                return 'Android'
        else:
                return 'other'
...
def draw_platform_data_saturation(results, spending_list, current_spending, media_list, Km):
        ...
        for item in ['iOS', 'Android', 'other']:
                ...
                for i in range(0, Km):
                        column_property = get_column_property(media_list[i])
                        if item == column_property:
                                plt.plot(spending_list, results[i], label=media_list[i])
                                plt.scatter(current_spending[i][0], current_spending[i][1], s=10)
